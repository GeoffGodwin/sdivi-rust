...
- **Pure-compute entry point in `sdi-core`:**
  ...
  - Algorithm: for each commit, enumerate every unordered pair of paths in the commit's `files`; increment a per-pair counter; divide each pair's count by `commits_analyzed`; emit pairs whose frequency `>= min_frequency`. Pairs are ordered by `(source, target)` lexicographically with `source < target`; output is a `Vec<CoChangePair>` sorted by `(source, target)` for byte-identical determinism.
...
- **New input/output types in `sdi-core::input`:**
  ...
  - `ChangeCouplingResult { pairs: Vec<CoChangePair>, commits_analyzed: u32, distinct_files_touched: u32 }`.
...
- Unit (pure-compute): empty events → empty pairs; single commit → no pairs (a pair requires two distinct commits); two commits each touching `{a, b}` → one pair `(a, b)` at frequency 1.0; out-of-window commits ignored when `events.len() > history_depth`.
- Unit (filter): three pairs with frequencies 0.8, 0.6, 0.4 against `min_frequency = 0.6` → exactly two pairs survive, sorted lexicographically.
...
- **`min_frequency = 0.0`.** Yields every pair that co-changed even once. Allowed; document the implication.

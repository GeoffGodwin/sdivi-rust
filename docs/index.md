...
**Structural Divergence Indexer.** A measurement instrument that tracks how
fast the structure of a codebase is changing.
...
> sdivi-rust is the Rust reimplementation of the Python POC (`sdi-py`).
> Snapshots are not backward compatible with the Python POC. See
> [Migrating from sdi-py](migrating-from-sdi-py).
...
| Migrate from the Python POC | [Migrating from sdi-py](migrating-from-sdi-py) |

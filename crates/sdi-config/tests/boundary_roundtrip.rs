...
fn to_yaml_round_trips() {
    ...
    let decoded: BoundarySpec = serde_yml::from_str(&yaml)
        .expect("to_yaml output must be valid YAML");
...
fn to_yaml_contains_names() {
    let spec = make_spec();
    let yaml = spec.to_yaml();
    assert!(yaml.contains("api"), "YAML must contain boundary name 'api'");
    assert!(yaml.contains("models"), "YAML must contain boundary name 'models'");
}
...
fn empty_spec_round_trips() {
    ...
    let yaml = spec.to_yaml();
    let decoded: BoundarySpec = serde_yml::from_str(&yaml)
        .expect("empty spec YAML must be parseable");

...
fn config_default_serde_round_trip_is_identity() {
    ...
    let decoded: Config =
        serde_json::from_str(&json).expect("serialized Config must deserialize");
    let json2 =
        serde_json::to_string(&decoded).expect("deserialized Config must re-serialize");
    assert_eq!(
        json, json2,
...
fn nested_config_fields_survive_round_trip() {
    ...
    assert!(
        (original.boundaries.leiden_gamma - decoded.boundaries.leiden_gamma).abs()
            < f64::EPSILON
    );
    assert_eq!(
    ...
    assert_eq!(original.boundaries.weighted_edges, decoded.boundaries.weighted_edges);
    assert!(
        (original.thresholds.pattern_entropy_rate - decoded.thresholds.pattern_entropy_rate)
            .abs()
            < f64::EPSILON
    );
    assert!(
...
fn config_default_is_deterministic_across_calls() {
    ...
    assert_eq!(first, second, "second Config::default() JSON differs from first");
    assert_eq!(first, third, "third Config::default() JSON differs from first");
}

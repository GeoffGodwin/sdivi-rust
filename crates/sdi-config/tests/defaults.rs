use sdi_config::{ColorChoice, Config, OutputFormat};
...
fn snapshot_defaults() {
    let cfg = Config::default();
    assert_eq!(cfg.snapshots.dir, ".sdi/snapshots");
...
fn boundaries_defaults() {
    let cfg = Config::default();
    assert_eq!(cfg.boundaries.spec_file, ".sdi/boundaries.yaml");
...
fn load_or_default_returns_defaults() {
    let cfg = sdi_config::load_or_default(std::path::Path::new(".")).unwrap();
    assert_eq!(cfg.core.random_seed, 42);
}

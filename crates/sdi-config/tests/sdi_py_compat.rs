...
use sdi_config::{load_with_paths, BoundarySpec};
...
fn sdi_py_boundaries_yaml_loads_cleanly() {
    ...
    let api = spec.boundaries.iter().find(|b| b.name == "api").expect("api boundary");
    assert_eq!(api.allow_imports_from, vec!["models", "utils"]);
    assert!(api.description.as_deref().is_some());

    let models = spec.boundaries.iter().find(|b| b.name == "models").expect("models boundary");
...
/// sdi-rust-only config sections ([determinism], [bindings]) are absent from
...
fn empty_config_file_succeeds() {
    ...
    let config = load_with_paths(Some(f.path()), None)
        .expect("empty config file must succeed");
...
fn boundary_spec_load_invalid_yaml_returns_boundary_parse_error() {
    ...
    let err = BoundarySpec::load(f.path())
        .expect_err("load of invalid YAML must return an error");

    match err {
        ConfigError::BoundaryParse(ref msg) => {
            assert!(!msg.is_empty(), "BoundaryParse error message must be non-empty");
        }
...
fn boundary_parse_error_message_is_non_empty() {
    ...
    let err = BoundarySpec::load(f.path())
        .expect_err("syntactically invalid YAML must return an error");

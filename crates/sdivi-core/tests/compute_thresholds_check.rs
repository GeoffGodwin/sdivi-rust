...
fn applied_overrides_reports_active_and_expired_separately() {
    ...
    assert!(
        r.applied_overrides["active_cat"].active,
        "active_cat override must be active"
    );
    assert!(
        !r.applied_overrides["expired_cat"].active,
        "expired_cat override must be inactive"
    );
    assert!(
        r.applied_overrides["expired_cat"].expired_reason.is_some(),
...
fn category_present_in_curr_only_surfaces_positive_delta() {
    ...
    assert!(
        r.breached,
        "newly-appearing category with delta > global rate must breach"
    );

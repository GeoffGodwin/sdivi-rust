//! Demonstrates programmatic `Config` construction for embedding `sdi-pipeline`.
//!
...
use anyhow::Result;
use sdi_config::{Config, ThresholdsConfig};
...
fn main() -> Result<()> {
    ...
    println!("Config: seed={} retention={}", config.core.random_seed, config.snapshots.retention);

    let pipeline = Pipeline::new(config, vec![Box::new(RustAdapter)]);
    let ts = current_timestamp();
    let snapshot = pipeline.snapshot_with_mode(

...
/// Canonical content of the default config written by `sdi init`.
...
/// `sdi init` in an empty directory creates `.sdi/config.toml`.
#[test]
fn init_creates_config_toml() {
    ...
    let config_path = dir.path().join(".sdi").join("config.toml");
    assert!(config_path.exists(), ".sdi/config.toml must be created");
}
...
fn init_config_toml_parses_to_defaults() {
    ...
    let config_path = dir.path().join(".sdi").join("config.toml");
    let config = sdi_config::load_with_paths(Some(&config_path), None)
    ...
    assert_eq!(config.snapshots.dir, ".sdi/snapshots");
...
fn init_config_content_matches_canonical() {
    ...
    let written = std::fs::read_to_string(dir.path().join(".sdi").join("config.toml"))
...
/// Running `sdi init` twice does not clobber the existing config.
#[test]
...
#[test]
fn init_respects_sdi_config_path_env_var() {
    let dir = TempDir::new().unwrap();
    ...
    sdi()
        ...
        .arg(dir.path())
        .arg("init")
        .env("SDI_CONFIG_PATH", &custom_config)
        .assert()
        .success();

    assert!(custom_config.exists(), "config must be written to SDI_CONFIG_PATH");
    assert!(
        !dir.path().join(".sdi").join("config.toml").exists(),
        ".sdi/config.toml must NOT be created when SDI_CONFIG_PATH is set"
    );
}

/// When an existing config at `SDI_CONFIG_PATH` has a missing `expires`, init exits 2.
#[test]
fn init_exits_2_for_missing_expires_in_existing_config() {
    ...
    sdi()
        ...
        .arg(dir.path())
        .arg("init")
        .env("SDI_CONFIG_PATH", &config_path)
        .assert()
        .failure()
        .code(2)

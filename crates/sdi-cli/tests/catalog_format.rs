//! Integration tests for `sdi catalog --format json|text`.
...
fn simple_rust_path() -> &'static str {
    concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/fixtures/simple-rust")
}

/// `sdi catalog --format json` exits 0 and outputs valid JSON to stdout.
#[test]
...
/// `sdi catalog --format json` emits nothing on stderr about parse errors.
#[test]
...
/// `sdi catalog` (text format, default) exits 0 and writes to stdout.
#[test]
...
/// `sdi catalog --format text` prints something to stdout.
#[test]
...
/// `sdi catalog --format json` with the high-entropy fixture produces a
/// catalog with at least one category entry.
#[test]
fn catalog_json_high_entropy_has_entries() {
    let high_entropy_path =
        concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/fixtures/high-entropy");

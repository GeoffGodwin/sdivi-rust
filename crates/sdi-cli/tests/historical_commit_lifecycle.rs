//! Full CLI integration: `sdi snapshot --commit REF` against a fixture repo.
//!
...
fn snapshot_commit_head_minus_one_labels_with_sha() {
    ...
    assert!(
        out.status.success(),
        "sdi snapshot --commit HEAD~1 must exit 0; stderr: {}",
    ...
    assert!(
        parsed["timestamp"].as_str().map(|s| s.ends_with('Z')).unwrap_or(false),
...
#[test]
fn snapshot_at_historical_commit_persisted_under_sdi_snapshots() {
    let repo = setup_two_commit_repo();
    ...
    let entries: Vec<_> = std::fs::read_dir(&snap_dir)
        .expect(".sdi/snapshots must exist after snapshot")
        .filter_map(|e| e.ok())
        .collect();
    assert_eq!(entries.len(), 1, "exactly one snapshot file must be persisted");
}

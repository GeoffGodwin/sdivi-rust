//! Tests for `sdi check --format json` with applied_overrides.
...
/// `sdi check --format json` always emits an `applied_overrides` field.
#[test]
...
fn applied_overrides_round_trips_in_json() {
    use sdi_core::compute::thresholds::{AppliedOverrideInfo, ThresholdCheckResult};
    ...
    assert_eq!(result, decoded, "ThresholdCheckResult must round-trip through JSON");
    assert!(decoded.applied_overrides["error_handling"].active);
    assert!(!decoded.applied_overrides["logging"].active);
    assert!(decoded.applied_overrides["logging"].expired_reason.is_some());
}

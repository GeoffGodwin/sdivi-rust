...
/// First-run `sdi check`: no prior snapshot → null DivergenceSummary → exit 0.
#[test]
...
/// `sdi check` with a prior snapshot on an empty (unchanged) repo exits 0.
#[test]
...
/// `sdi check --no-write` does not create a snapshot file.
#[test]
fn check_no_write_creates_no_snapshot() {
    ...
    assert_eq!(
        before, after,
        "sdi check --no-write must not create snapshot files"
    );
}

/// `sdi check` (with write, no prior) creates exactly one snapshot.
#[test]
fn check_with_write_creates_one_snapshot() {
    ...
    sdi()
        ...
        .assert()
        .success();
    assert_eq!(snapshot_count(&repo), 1, "sdi check must write exactly one snapshot");
}

/// `sdi check --format json` emits valid JSON with `exit_code` field.
#[test]
fn check_json_has_exit_code_field() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(&stdout)
        .expect("check --format json stdout must be valid JSON");
...
/// `sdi check --format json | .exit_code` returns the same value as the process exit code.
#[test]

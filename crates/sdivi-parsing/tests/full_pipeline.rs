...
//! compile tests there. This file lives in `crates/sdivi-parsing/tests/` which
//! has the required dev-dependencies via sdivi-parsing's Cargo.toml.
...
fn parse_same_fixture_twice_identical() {
    let config = Config::default();
    let make_adapters =
        || -> Vec<Box<dyn sdivi_parsing::adapter::LanguageAdapter>> { vec![Box::new(RustAdapter)] };
    ...
    for (a, b) in first.iter().zip(second.iter()) {
        assert_eq!(
            a.imports, b.imports,
            "imports must be deterministic for {:?}",
            a.path
        );
        assert_eq!(
            a.exports, b.exports,
            "exports must be deterministic for {:?}",
            a.path
        );
        assert_eq!(
            a.signatures, b.signatures,
            "sigs must be deterministic for {:?}",
            a.path
        );
    }
}
...
fn parse_empty_dir_returns_no_records() {
    ...
    assert!(
        records.is_empty(),
        "empty directory must produce zero records"
    );
}

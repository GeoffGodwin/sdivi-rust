...
fn node_path_returns_path_for_valid_index() {
    ...
    let idx0 = dg.node_for_path(&PathBuf::from("src/lib.rs"))
        .expect("lib.rs must be in graph");
    let idx1 = dg.node_for_path(&PathBuf::from("src/models.rs"))
        .expect("models.rs must be in graph");

    assert_eq!(dg.node_path(idx0), Some(PathBuf::from("src/lib.rs").as_path()));
    assert_eq!(dg.node_path(idx1), Some(PathBuf::from("src/models.rs").as_path()));
}
...
fn node_path_returns_none_for_out_of_range_index() {
    ...
    let dg = build_dependency_graph(&records);
    assert!(dg.node_path(999).is_none(), "out-of-range index must return None");
}
...
fn relative_import_dot_slash_resolves_to_sibling() {
    ...
    let dg = build_dependency_graph(&records);
    assert_eq!(dg.edge_count(), 1, "relative ./utils must resolve to src/utils.py");
}
...
fn relative_import_parent_slash_strips_prefix_resolves_in_same_dir() {
    ...
    let dg = build_dependency_graph(&records);
    assert_eq!(dg.edge_count(), 1, "../shared strips prefix and resolves in same directory");
}
...
fn relative_import_unresolvable_drops_silently() {
    ...
    let dg = build_dependency_graph(&records);
    assert_eq!(dg.edge_count(), 0, "unresolvable relative import must be dropped");
}
...
fn crate_prefix_resolves_first_path_segment_only() {
    ...
    let dg = build_dependency_graph(&records);
    assert_eq!(dg.edge_count(), 1, "crate::models::User must resolve to models.rs");
}
...
fn super_prefix_resolves_to_stem() {
    ...
    let dg = build_dependency_graph(&records);
    assert_eq!(dg.edge_count(), 1, "super::config must resolve to config.rs");
}

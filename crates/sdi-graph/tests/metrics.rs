...
fn simple_rust_fixture_five_nodes_zero_edges() {
    ...
    assert_eq!(dg.edge_count(), 0, "simple-rust fixture must have 0 edges (all imports external)");
}
...
fn duplicate_import_does_not_add_duplicate_edge() {
    ...
    let dg = build_dependency_graph(&records);
    assert_eq!(dg.edge_count(), 1, "duplicate import must produce only one edge");
}

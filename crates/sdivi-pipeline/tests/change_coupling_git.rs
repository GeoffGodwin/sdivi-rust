...
use sdivi_pipeline::change_coupling::collect_cochange_events;
...
fn path_with_space_round_trips() {
    ...
    write_and_commit(
        &dir,
        &[("src/my file.rs", "x"), ("src/b.rs", "y")],
        "space in name",
    );
    write_and_commit(
        &dir,
        &[("src/my file.rs", "x2"), ("src/b.rs", "y2")],
        "space again",
    );

    let events = collect_cochange_events(&dir, 100, None).unwrap();
    let has_space = events
        .iter()
        .any(|e| e.files.iter().any(|f| f.contains("my file")));

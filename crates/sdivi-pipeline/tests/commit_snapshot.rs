...
fn adapters() -> Vec<Box<dyn sdivi_parsing::adapter::LanguageAdapter>> {
...
fn different_commits_produce_distinct_snapshots() {
    ...
    assert_ne!(
        head.commit, head1.commit,
        "HEAD and HEAD~1 have different SHAs"
    );
    assert_ne!(
        head1.commit, head2.commit,
        "HEAD~1 and HEAD~2 have different SHAs"
    );
...
fn nonexistent_ref_returns_error() {
    ...
    assert!(result.is_err(), "nonexistent ref must produce an error");

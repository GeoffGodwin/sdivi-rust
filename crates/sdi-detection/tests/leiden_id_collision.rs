...
fn leiden_all_nodes_in_community_zero_no_underflow() {
    ...
    assert_eq!(partition.assignments.len(), n, "all {n} nodes must be assigned");
    assert!(partition.community_count() >= 1);
}
...
fn leiden_multi_iteration_no_underflow_on_two_cliques() {
    ...
    let cfg = LeidenConfig { seed: 42, max_iterations: 10, ..LeidenConfig::default() };
    let partition = run_leiden(&dg, &cfg, Some(&warm_start));

    assert_eq!(partition.assignments.len(), 10, "all 10 nodes must be assigned");
    assert!(partition.community_count() >= 1, "at least one community must exist");
    let k = partition.community_count();
    for (&node, &comm) in &partition.assignments {
        assert!(comm < k, "node {node}: community {comm} out of range [0, {k})");
    }

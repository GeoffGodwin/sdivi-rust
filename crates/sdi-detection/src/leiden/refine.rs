...
fn refine_community(
    ...
    let member_set: std::collections::BTreeSet<usize> =
...
fn move_node_to(node: usize, to: usize, refined: &mut Vec<usize>) {
    refined[node] = to;
}

/// Renumbers community IDs in-place to a dense range `[0, k)`.
fn renumber_in_place(assignment: &mut Vec<usize>) {

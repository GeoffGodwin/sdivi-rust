...
use sdi_pipeline::commit_extract::{CommitExtractError, resolve_ref_to_sha, extract_commit_tree};
    ...
    match result {
        Err(CommitExtractError::RefResolutionFailed { reference, stderr }) => {
            ...
            assert!(!stderr.to_lowercase().contains("unknown option"),
                   "stderr should not indicate unknown option (missing -- separator): {}", stderr);
        }
...
fn create_test_git_repo() -> Result<(tempfile::TempDir, String), Box<dyn std::error::Error>> {
    ...
    if !init_out.status.success() {
        return Err(format!("git init failed: {}", String::from_utf8_lossy(&init_out.stderr)).into());
    }
    ...
    if !add_out.status.success() {
        return Err(format!("git add failed: {}", String::from_utf8_lossy(&add_out.stderr)).into());
    }
    ...
    if !commit_out.status.success() {
        return Err(format!("git commit failed: {}", String::from_utf8_lossy(&commit_out.stderr)).into());
    }
...
fn stderr_truncation_prevents_information_leakage() {
    ...
    match result {
        Err(CommitExtractError::RefResolutionFailed { stderr, .. }) => {
            ...
            assert!(stderr.len() <= 210,
                   "stderr should be truncated to ~200 chars, got {} chars: {}",
                   stderr.len(), stderr);
            // Should not contain null bytes
            assert!(!stderr.contains('\0'), "stderr should not contain null bytes");
            // Should not be empty
...
fn extract_commit_tree_succeeds_with_tar_flags() {
    ...
    let (tmpdir, sha) = create_test_git_repo()
    ...
    assert!(test_file.exists(), "test.txt should exist in extracted tree");

    // Verify content is correct
    let content = std::fs::read_to_string(&test_file)
        .expect("should be able to read test.txt");
    assert_eq!(content, "test content", "test.txt should have original content");

...
fn snapshot_populates_change_coupling() {
    ...
    // Create .sdi/snapshots dir.
    std::fs::create_dir_all(dir.join(".sdi/snapshots")).unwrap();
    ...
    let snap = pipeline.snapshot_with_mode(
        ...
        sdi_pipeline::WriteMode::EphemeralForCheck,
    ).unwrap();
...
fn no_git_produces_none_change_coupling() {
    ...
    std::fs::create_dir_all(tmp.path().join(".sdi/snapshots")).unwrap();

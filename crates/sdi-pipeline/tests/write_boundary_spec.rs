//! Tests for `sdi_pipeline::store::write_boundary_spec`.
//!
...
fn existing_file_with_inline_hash_is_overwritten() {
    ...
    let original_with_inline_comment =
        "boundaries: [] # auto-generated\n";
...
fn write_creates_parent_directories() {
    let dir = TempDir::new().unwrap();
    let nested = dir.path().join(".sdi").join("config").join("boundaries.yaml");
    ...
    assert!(nested.exists(), "file must be created inside new parent dirs");
}

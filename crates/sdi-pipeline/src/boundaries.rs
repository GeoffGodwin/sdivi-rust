...
//! slices, and delegates to `sdi_core::infer_boundaries`.

use std::path::Path;

use sdi_snapshot::boundary_inference::{BoundaryInferenceResult, PriorPartition};
...
/// version of sdi-rust or by the pure-compute path) are included as empty
...
pub fn infer_from_snapshots(
    ...
    Ok(sdi_core::infer_boundaries(&partitions, stability_threshold))
}
...
mod tests {
    ...
    fn write_fake_snapshot(dir: &TempDir, name: &str, pp: BTreeMap<String, u32>) {
        ...
        use sdi_snapshot::snapshot::{PatternMetricsResult, assemble_snapshot};
        ...
        let snap_dir = dir.path().join(".sdi").join("snapshots");
    ...
    fn stable_community_is_proposed() {
        ...
        let result = infer_from_snapshots(&snap_dir, 2).unwrap();
        assert!(!result.proposals.is_empty(), "expected at least one proposal");
    }

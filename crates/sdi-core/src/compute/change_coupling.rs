...
///
/// ```rust
/// use sdi_core::input::{CoChangeEventInput, ChangeCouplingConfigInput};
...
pub fn compute_change_coupling(
    ...
    let mut pairs: Vec<CoChangePair> = pair_counts
        ...
        .filter_map(|((source, target), count)| {
            ...
            Some(CoChangePair { source, target, frequency, cochange_count: count })
        })
        .collect();
    ...
    Ok(ChangeCouplingResult { pairs, commits_analyzed, distinct_files_touched })
}

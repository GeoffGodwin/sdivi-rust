...
use sdi_core::input::{DependencyGraphInput, EdgeInput, LeidenConfigInput, NodeInput};
...
fn three_disconnected_components_detected() {
    ...
    assert_eq!(r.disconnected_components, 3, "expected 3 disconnected components");
    assert!(r.community_count >= 3, "should detect at least 3 communities");
}
...
fn connected_graph_one_component() {
    let g = DependencyGraphInput {
        ...
        edges: vec![edge("a.rs", "b.rs"), edge("b.rs", "c.rs"), edge("c.rs", "a.rs")],
    };
    let r = detect_boundaries(&g, &LeidenConfigInput::default(), &[]).unwrap();
    assert_eq!(r.disconnected_components, 1);
}

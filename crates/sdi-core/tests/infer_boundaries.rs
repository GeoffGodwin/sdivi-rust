//! Pure-compute tests for `sdi_core::infer_boundaries`.
//!
...
use sdi_core::{infer_boundaries, SnapshotPriorPartition as PriorPartition};
...
fn single_partition_no_stable_pairs() {
    ...
    assert!(result.proposals.is_empty(), "need at least 2 partitions to form a stable pair");
}
...
fn three_identical_partitions_threshold_two() {
    ...
    assert_eq!(result.proposals.len(), 2, "both communities should be proposed");

    let comm_0 = result.proposals.iter().find(|p| p.node_ids == vec!["a.rs", "b.rs"]);
    let comm_1 = result.proposals.iter().find(|p| p.node_ids == vec!["c.rs", "d.rs"]);
    assert!(comm_0.is_some(), "community with {{a.rs, b.rs}} must be proposed");
    assert!(comm_1.is_some(), "community with {{c.rs, d.rs}} must be proposed");
}
...
fn threshold_not_met() {
    ...
    assert!(result.proposals.is_empty(), "only 1 pair available, threshold=3");
}
...
fn community_renumbering_handled() {
    ...
    assert!(
        result.proposals.iter().any(|p| p.node_ids == vec!["a.rs", "b.rs"]),

...
use sdi_core::input::{ThresholdsInput, ThresholdOverrideInput};
...
fn default_global_rates() {
    let input = ThresholdsInput::default();
    assert_eq!(input.pattern_entropy_rate, 2.0, "Default pattern_entropy_rate");
    assert_eq!(input.convention_drift_rate, 3.0, "Default convention_drift_rate");
    assert_eq!(input.coupling_delta_rate, 0.15, "Default coupling_delta_rate");
    assert_eq!(

...
fn threshold_met_community_proposed() {
    ...
    let found = r.proposals.iter().any(|p| p.node_ids.contains(&"a.rs".to_string()));
    assert!(found);
}
...
fn partition_count_matches_input_length() {
    let partitions: Vec<PriorPartition> = (0..5)
        .map(|_| partition_from(&[("x.rs", 0)]))
        .collect();
...
fn proposals_sorted_by_community_id() {
    let make = || {
        partition_from(&[("a.rs", 2), ("b.rs", 2), ("c.rs", 0), ("d.rs", 0)])
    };
    let r = infer_boundaries(&[make(), make(), make()], 2);

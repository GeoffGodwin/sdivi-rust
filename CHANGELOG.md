...
### Added
- `crates/sdi-pipeline/src/commit_extract.rs` (NEW) — ref resolution, UTC date normalization, tree extraction via git archive | tar (M16)

### Changed
- **M16 — `sdi snapshot --commit REF` now analyzes the actual tree at REF.**
...
- **`CommitExtractError`** added to `sdi_pipeline::PipelineError` as
...
### Added
- **M15 — Change-coupling analyzer**: New snapshot field `change_coupling`. New `boundaries.weighted_edges = true` mode multiplies import-edge weights by `(1.0 + frequency)`. New pure-compute entry point `sdi_core::compute_change_coupling` exported through WASM. Schema stays `"1.0"`.

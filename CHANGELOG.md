...
- `boundaries.leiden_min_compression_ratio` config key (default `0.1`): stop Leiden recursion when an aggregation level would compress the graph by less than this fraction of nodes.
- `boundaries.leiden_max_recursion_depth` config key (default `32`): hard cap on Leiden recursion depth. Belt-and-suspenders safeguard; `32` corresponds to ~4 billion nodes.
- Both new keys are optional in existing `.sdivi/config.toml` files — old configs deserialize with the defaults transparently.
- `LeidenConfig::min_compression_ratio` and `LeidenConfig::max_recursion_depth` fields.
...
### Added
- [MILESTONE 26 ✓] feat: Implement Milestone 26: Resolver — Parent Navigation and Per-Language (M27)
## [0.2.14] - 2026-05-04
...
- npm package now ships `LICENSE`, `NOTICE`, and `README.md` at the
  package root. Previously the outer `pkg/package.json` declared
  these in its `files` field but the build process never copied
  them into `pkg/` — only inner `bundler/README.md` and
  `node/README.md` (auto-generated by wasm-pack) were present.
  License-compliance scanners (Artifactory Xray, etc.) flag a
  package that declares a license in metadata but ships no LICENSE
  text as a quarantine candidate. Both `bindings/sdivi-wasm/build.sh`
  (local dev) and `.github/workflows/release.yml` + `wasm.yml`
  (CI) now copy these files into `pkg/` after wasm-pack runs.

### Fixed

- `Cargo.toml` `[workspace.package]` `repository` URL was lowercase
  (`https://github.com/geoffgodwin/sdivi-rust`); fixed to canonical
  case (`https://github.com/GeoffGodwin/sdivi-rust`). This propagates
  into wasm-pack-generated inner `package.json` files (one per
  bundler/ and node/ subdir), eliminating a metadata-inconsistency
  signal between the outer manifest (already canonical) and the
  inner ones (previously lowercase). The lowercase variant also
  appears in `bindings/sdivi-wasm/README.md`; updated there too.

### Added

- `wasm.yml` `Verify npm pack` step now also asserts `LICENSE`,
  `NOTICE`, and `README.md` are present in the tarball — guards
  against silent regression of the license-text-shipped invariant.

### Notes

- These changes are aimed at reducing the surface area for
...
- crates.io publish: keyword `"structural-divergence"` (22 chars)
  exceeded crates.io's 20-character keyword limit, blocking the
  first crate (`sdivi-config`) at publish time. Replaced with
  `"code-divergence"` (15 chars) across all 14 per-crate
  `Cargo.toml` files; also synced the same keyword in the WASM
  package.json files for consistency.

## [0.2.6] - 2026-05-04

### Fixed

- npm publish: `bindings/sdivi-wasm/pkg-template/package.json`
  `repository.url` was lowercase (`geoffgodwin/sdivi-rust`); the
  provenance attestation generated by GitHub Actions uses the real-
  case username (`GeoffGodwin/sdivi-rust`) and sigstore validation
  rejected v0.2.5 with a 422 mismatch error. Updated to
  `git+https://github.com/GeoffGodwin/sdivi-rust.git` (canonical
  npm-normalized form). Also matches `cargo publish`'s expected
  format.
- Crates.io publish in v0.2.5 was blocked by a missing email-verify
  on the publishing crates.io account. Email is now verified;
  publishes flow normally from this release onward.

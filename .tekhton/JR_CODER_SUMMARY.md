...
1. **`crates/sdivi-core/src/input/edge_weight.rs:9–14`** — Updated `edge_weight_key` doc comment
...
2. **`bindings/sdivi-wasm/src/types.rs:46`** — Strengthened `WasmLeidenConfigInput` doc comment
...
3. **`crates/sdivi-pipeline/src/change_coupling.rs:86–88`** — Clarified `parse_git_log_output` doc comment
...
4. **`crates/sdivi-detection/src/leiden/quality.rs:1`** — Added missing module-level doc
...
1. **`crates/sdivi-core/src/compute/boundaries.rs:160,173–174`** — Removed unused stub code
...
2. **`crates/sdivi-patterns/src/catalog.rs:133,146`** — Added feature gates to dead code in `--no-default-features` build
...
3. **`tests/historical_commit_lifecycle.rs`** — Deleted comment-only placeholder file
   - File was 7 lines: comment explaining the actual test is at `crates/sdivi-cli/tests/historical_commit_lifecycle.rs`
